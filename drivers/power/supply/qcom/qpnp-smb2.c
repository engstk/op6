...
static int ech_wls_chrg_get_psy_info(enum power_supply_property psp, int *data)
{
	...
	if (psy) {
		if (psy->desc->get_property) {
			if (!psy->desc->get_property(psy, psp, &ret)) {
				*data = ret.intval;
				return 0;
			}
		}
...
static int ech_wls_is_online(void)
{
	int data = 0;

	ech_wls_chrg_get_psy_info(POWER_SUPPLY_PROP_ONLINE, &data);
	return data;
}
